...
ContractHistorySchema.pre("save", function (next) {
  ...
  switch (this.eventType) {
    case "PAYMENT_MADE":
      if (
        !this.changeDetails.paymentInfo ||
        !this.changeDetails.paymentInfo.amount
      ) {
        return next(
          new Error("La información de pago es requerida para eventos de pago")
        );
      }
      break;
    ...
    case "PHASE_CHANGE":
      if (!this.changeDetails.previousPhase && !this.changeDetails.newPhase) {
        return next(
          new Error(
            "Se debe especificar la fase anterior o nueva para cambios de fase"
          )
        );
      }
      break;
    ...
    case "DOCUMENT_REJECTION":
      if (!this.changeDetails.documentInfo) {
        return next(
          new Error(
            "La información del documento es requerida para eventos de documentos"
          )
        );
      }
      break;
...
ContractHistorySchema.statics.findByEventType = function (
  eventType,
  options = {}
) {
...
ContractHistorySchema.statics.getRecentActivity = function (
  contractId,
  hours = 24
) {
...
export const ContractHistory = mongoose.model(
  "ContractHistory",
