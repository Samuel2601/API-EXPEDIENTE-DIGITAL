...
const PhaseControlJSON = {
  ...
  status: {
    type: String,
    enum: {
      values: ["PENDING", "IN_PROGRESS", "COMPLETED", "NOT_APPLICABLE", "BLOCKED", "CANCELLED"],
      message: "Estado de fase no válido"
    },
    default: "PENDING",
    uppercase: true,
    meta: {
      validation: { isIn: ["PENDING", "IN_PROGRESS", "COMPLETED", "NOT_APPLICABLE", "BLOCKED", "CANCELLED"], optional: true },
      messages: {
...
export const ContractJSON = {
  contractNumber: {
    ...
    maxlength: 50,
    meta: {
      validation: { isString: true, required: true, notEmpty: true, isLength: { min: 3, max: 50 } },
      messages: {
  ...
  contractualObject: {
    ...
    maxlength: 1000,
    meta: {
      validation: { isString: true, required: true, notEmpty: true, isLength: { min: 10, max: 1000 } },
      messages: {
  ...
  generalStatus: {
    ...
    enum: {
      values: [
        "PREPARATION", "CALL", "EVALUATION", "AWARD", "CONTRACTING", 
        "EXECUTION", "RECEPTION", "LIQUIDATED", "FINISHED", "DESERTED", "CANCELLED"
      ],
    ...
    index: true,
    meta: {
      validation: { isIn: ["PREPARATION", "CALL", "EVALUATION", "AWARD", "CONTRACTING", "EXECUTION", "RECEPTION", "LIQUIDATED", "FINISHED", "DESERTED", "CANCELLED"], optional: true },
      messages: {
...
ContractSchema.pre('save', async function(next) {
  ...
  if (timeline.executionStartDate && timeline.executionEndDate) {
    if (timeline.executionStartDate >= timeline.executionEndDate) {
      return next(new Error('La fecha de inicio de ejecución debe ser anterior a la fecha de fin'));
    }
  }
  
  if (timeline.questionsDeadline && timeline.submissionDeadline) {
    if (timeline.questionsDeadline >= timeline.submissionDeadline) {
      return next(new Error('La fecha límite de preguntas debe ser anterior a la de presentación'));
    }
  }
  
  // Validar valores presupuestarios
  if (this.budget.awardedValue && this.budget.estimatedValue) {
    const variation = (this.budget.awardedValue / this.budget.estimatedValue) * 100;
    if (variation > 150) { // 50% más que el estimado
      console.warn(`Contrato ${this.contractNumber}: Valor adjudicado excede significativamente el estimado`);
    }
  }
  
  if (this.budget.paidValue > (this.budget.awardedValue || this.budget.estimatedValue)) {
    return next(new Error('El valor pagado no puede exceder el valor adjudicado o estimado'));
  }
...
ContractSchema.methods.getCurrentPhaseInfo = function() {
  ...
  return this.phases.find(p => 
    p.phase.toString() === this.currentPhase.toString()
  );
};

ContractSchema.methods.getNextPhase = function() {
  const currentPhaseIndex = this.phases.findIndex(p => 
    p.phase.toString() === this.currentPhase?.toString()
  );
...
ContractSchema.query.inProgress = function() {
  return this.where({
    generalStatus: { $in: ['PREPARATION', 'CALL', 'EVALUATION', 'CONTRACTING', 'EXECUTION'] }
  });
};
